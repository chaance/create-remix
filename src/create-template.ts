...
export async function createTemplate(
	...
	try {
		if (isRemixTemplate(template)) {
			await log(
				`Using the ${template} template from the "remix-run/remix" repo`
			);
		...
		if (isRemixStack(template)) {
			...
			await log(`Using the template from the "remix-run/${stack}" repo`);
			await createTemplateFromRemixStack(stack, destPath, options);
			return;
		}

		if (isRemixExample(template)) {
			await log(`Using the template from the "remix-run/examples" repo`);
		...
		if (isLocalFilePath(template)) {
			await log(`Using the template from local file at "${template}"`);
			let filepath = template.startsWith("file://")
				? url.fileURLToPath(template)
				: template;
			await createTemplateFromLocalFilePath(filepath, destPath, options);
			return;
		}

		if (isGithubRepoShorthand(template)) {
			await log(`Using the template from the "${template}" repo`);
		...
		if (isUrl(template)) {
			await log(`Using the template from "${template}"`);
...
async function createTemplateFromRemoteTarball(
	...
	return await downloadAndExtractRepoTarball(
		getRepoInfo(url),
		destPath,
		options
	);
...
async function createTemplateFromLocalFilePath(
	...
	if (filePath.endsWith(".tar.gz")) {
		await extractLocalTarball(filePath, destPath, options);
		return;
	}
...
interface DownloadAndExtractTarballOptions extends CreateTemplateOptions {
	filePath?: string | null;
}
...
async function downloadAndExtractTarball(
	downloadPath: string,
	tarballUrl: string,
	{ token, filePath, onError }: DownloadAndExtractTarballOptions
): Promise<void> {
	let resourceUrl = tarballUrl;
	let headers: HeadersInit = {};
	if (token && new URL(tarballUrl).host.endsWith("github.com")) {
